...
`main.py`（コンソール版）と `ui_kohaku.py`（GUI版）の両方から同じパイプラインが動きます。
T1〜T7が繋がり、実ファイルでの通しテストに成功しています。
...
T7 t7_search.search              自然文で意味検索
   ui_kohaku                     常駐キャラ＋チャットUI
```

T1〜T6の受け渡しは `pipeline.process_file()` に切り出してあります。
コンソール版とGUI版が同じ順序を共有するためで、**順序を変えるときは pipeline.py を直すこと。**

**T8（放置ファイル検出）・T9（ゴミ箱Undo）は未着手です。**

### UIの方針（相談して決めたもの）

デスクトップ左下に常駐する相棒キャラ（コハク）がいて、2つの面を持ちます。

- **自発**: ダウンロードを検出したら吹き出しで「どこに入れたか」を知らせる（5秒で消える）
- **ユーザー始動**: クリックでチャットパネルが開き、自然文でファイルを探せる

検索結果も分類通知も訂正も、**すべて1本の会話ログに流します。** 画面を1つ作るだけで
T7・T8・T9が同じ場所に乗るのが狙いです。タブで画面を分ける構成は採りません。

決まっていること:

- **ファイルビューアは自前で持たない。** 「開く」はOSの既定アプリに投げるだけ
  （エクスプローラーでダブルクリックと同じ）。PDFビューアもページ送りも作りません
- **エクスプローラーの代替は作らない。** ツリービューもリネームも範囲外です
- **分類の訂正は必須。** AI分類は必ず外れるので、通知から直接カテゴリを入れ直せます
  （`t6_filemanager.recategorize()`）。ただし訂正を学習する仕組みは未実装

触れるモックはここ: https://claude.ai/artifact/EoA4bCxAxf9P315zsk4pd2

### GUIは PyQt6 ではなく PySide6

**この環境ではPyQt6のDLLがWindowsのアプリケーション制御ポリシーにブロックされ、
importすら通りません。** PySide6はQt公式の署名付きバイナリなので同じ環境で動きます。
...
検索クエリのベクトル化は `t6_filemanager.embed_query()` に統一済みです（T7はこれを呼んでいます）。
...
### (C) 検索のしきい値と時間表現（実装済み・値は要調整）

`t7_search.py` の定数まわりは実測で決めた暫定値です。件数が増えたら見直してください。

- `MIN_SCORE = 0.82`: e5のコサイン類似度は**無関係な文でも0.78前後まで出る**ため、
  絶対値だけでは切れません。実測では正解0.85〜0.91／無関係0.78〜0.79に分かれました
- `RELATIVE_MARGIN = 0.06`: 1位から離れた結果を捨てる相対的な足切り。
  「1件だけ強く当たっている」ときに惜しくない2位以下を並べないためのものです
- 時間表現（「先週」等）は正規表現でメタデータの絞り込み条件に変換しています。
  素のembedding検索に「先週」を投げても効かないためです。**「先週」は暦週ではなく
  4〜11日前**としています。暦どおりに切ると、水曜に「先週の資料」と言われたときに
  8日前を取りこぼすからです。この解釈をLLMに任せる場合も、`parse_query()` の
  戻り値の形さえ揃えれば `search()` から下はそのまま使えます
