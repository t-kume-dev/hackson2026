...
**見た目は白黒のドット絵で統一しています**（最初のモックのオレンジ・角丸の路線から変更）。

- 吹き出し・パネル・カードは黒地に白枠・白文字。角丸・影・グラデーションは使わない
- 色を使わないので、強調は白黒反転で表す（`<b>` を渡すと自動で反転の帯になる）
- フォントは2系統。コハクの「声」（吹き出し・ボタン・タイトル）は同梱の
  `assets/fonts/DotGothic16-Regular.ttf`（SIL OFL）を**14px以上**で使う。
  ファイル名・要約など情報の多い部分は普通のUIフォント。全部ドット絵フォントに
  したら見づらくて不評だった。ドット絵フォントの太字は滲むので使わない
- 白枠で囲むのはコハクの吹き出しだけ。カードは枠なしで面の明るさで区切り、
  主ボタン以外は細い灰色枠にする（全部白枠にしたら見づらくて不評だった）
- キャラは丸くて下ぶくれ、ほっぺ付き、口の大きなドット絵。普段は呼吸で上下して
  時々まばたきし、分類中は「> <」の目で口をあんぐり開けて右から飛んでくる
  ファイルを食べ、食べ終わると「^ ^」の顔で少し弾む。コマは `ui_kohaku.py` の `SPRITE_*` に
  文字列で書いてあるので、見た目の調整はそこを書き換えるだけでよい

最初の方向性を相談したときのモック（配色は古い）: https://claude.ai/artifact/EoA4bCxAxf9P315zsk4pd2
